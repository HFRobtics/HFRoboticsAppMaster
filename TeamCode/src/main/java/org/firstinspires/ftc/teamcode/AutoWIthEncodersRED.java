 ...
 */
@Autonomous(name="RED Auto Drive By Encoder", group="Robot")
public class AutoWIthEncodersRED extends LinearVisionOpMode {
    ...
    public void runOpMode() throws InterruptedException{ //Runs through once
        ...
        waitForStart(); //Wait for driver to start autonomo*9us/


        //Drives at (DRIVE_SPEED, distance in inches, direction, timeout in seconds)
        encoderDrive(DRIVE_SPEED,  39 + robot.LENGTH, RobotDirectionDrive.BACK, 30.0); //Drives to beacon
        robot.waitForTick(500);
        encoderDrive(DRIVE_SPEED,  16, RobotDirectionDrive.RIGHT, 30.0);
        robot.waitForTick(500);
        encoderDrive(DRIVE_SPEED, 3.625 + (robot.LENGTH /2.0), RobotDirectionDrive.BACK, 30.0);
        robot.waitForTick(500);
    ...
    public void encoderDrive(double speed,
        ...
        robot.frontleftMotor.setPower(Math.abs(speed)); //Front motors are run at .75 speed due to gearing
        ...
        while (opModeIsActive() &&
            ...
            telemetry.addData("front left: ", robot.backleftMotor.getPower());
            telemetry.addData("front left: ", robot.frontleftMotor.getPower());
            telemetry.update();
        }
    ...
    public Servo getLeftOrRightColor(){ //Returns servo to put down based on color gotten from beacon\
        ...
        if(left.toLowerCase().equals("red")){
            return robot.leftBeacon;
        }
        if(right.toLowerCase().equals("red")){
